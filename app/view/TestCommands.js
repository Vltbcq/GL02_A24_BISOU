...
function addTestCommands(program) {
  ...
  program
    ...
    .argument(
      "[questionsIds...]",
      "The ids of the initial questions of the test, space between ids. ex : '0 1 2 3'  will add question 1 to 4",
      []
    )
    .action((questionsIds = []) => {
    ...
    .action(async (id) => {
      ...
      for (const question of test.questions) {
          ...
          answer = await inquirer.prompt([
            {
              type: "input",
              name: "userAnswer",
              message: `<span class="math-inline">\{question\.question\}\\n</span>{question.textPart1} .... ${question.textPart2}`,
            },
          ]);
        } else {
          console.log(
            `Unknown question type for question: ${question.question}`
          );
          continue;
        }
        ...
        if (question instanceof TrueFalseQuestion) {
          ...
          isCorrect =
            question.correctAnswers.length === answer.userAnswer.length &&
            question.correctAnswers.every(
              (val, index) => val === answer.userAnswer[index]
            );
        } else if (question instanceof NumericQuestion) {
          ...
          isCorrect =
            question.answer &&
            answer.userAnswer.toString().toLowerCase() ===
        ...
        results.push({
          ...
          correctAnswer:
            question instanceof MultipleChoiceQuestion
              ...
              : question instanceof BlankWordQuestion
              ? question.blankWord
              : question.answer,
          isCorrect: isCorrect,
        });
      ...
      results.forEach((result) => {
        table.push([
          result.isCorrect ? chalk.green("Correct") : chalk.red("Incorrect"),
          result.question,
          result.userAnswer,
      ...
      console.log(
        color(
          `Score : ${score}/${test.questions.length} (${percentage.toFixed(
            2
          )}%)`
        )
      );
